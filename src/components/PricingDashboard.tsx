...
import { getProducts, saveProducts } from '../lib/api';
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 text-slate-900">
        <div className="bg-white border border-red-100 shadow-xl rounded-2xl p-8 max-w-md">
          <p className="text-sm font-bold text-red-600 uppercase mb-2">Erro</p>
          <h1 className="text-2xl font-bold mb-3">Nao foi possivel carregar ou salvar.</h1>
          <p className="text-slate-500 mb-6">{error}</p>
          <button onClick={onLogout} className="px-5 py-3 bg-slate-800 text-white rounded-xl font-bold">Voltar ao login</button>
        </div>
      </div>
    );
  }
      ...
      <footer className="h-8 bg-slate-800 flex items-center px-6 justify-between shrink-0">
        <div className="flex items-center space-x-4">
          <span className="flex items-center space-x-1.5">
            ...
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{isSaving ? 'Salvando no banco...' : 'Banco de dados conectado'}</span>
          </span>
        </div>
